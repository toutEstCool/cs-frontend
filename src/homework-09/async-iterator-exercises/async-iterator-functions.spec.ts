import { sleep } from '../../homework-08/index';
import take from './async-iterator-functions/take';
import map from './async-iterator-functions/map';
import filter from './async-iterator-functions/filter';
import seq from './async-iterator-functions/seq';
import any from './async-iterator-functions/any';
import every from './async-iterator-functions/every';
import repeat from './async-iterator-functions/repeat';
...
describe('Implementation of any - awaits the first resolved async iterator', () => {
  it('Any must return value of the first resolved async iterator', async () => {
    async function* asyncTasksLong() {
      await sleep(2000);
      yield 'Long';
    }

    async function* asyncTasksShort() {
      await sleep(200);
      yield 'Short';
    }

    for await (const value of any(asyncTasksLong(), asyncTasksShort())) {
